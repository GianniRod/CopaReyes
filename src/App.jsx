...
const simulateQuickMatch = (probA, probB, startMinute = 0, currentScoreA = 0, currentScoreB = 0) => {
    ...
    for (let m = startMinute; m < 90; m++) {
        ...
        if (random < 0.22) {
            ...
            // 30% increased goal probability
            let goalChance = (baseGoalChance + (diff * 0.45)) * 1.30;
...
export default function App() {
            ...
            if (Math.random() < (0.22 * fatigueFactor)) {
                ...
                if (Math.random() < 0.8) {
                    ...
                    if (Math.random() < 0.35) {
                        ...
                        const diff = isTeamA ? probA - probB : probB - probA;
                        // 30% increased goal probability
                        let goalChance = (0.35 + (diff * 0.7)) * 1.30; // Antes 0.3
    ...
    const updateMatchAddedTime = async (matchId, newAddedTime) => {
        if (!user) return;
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'matches', matchId), {
            addedTime: parseInt(newAddedTime)
        });
    };
                                ...
                                {match.status !== 'finished' && (
                                    <div className="bg-white border-l-4 border-l-[#009B3A] rounded-xl p-4 shadow-sm flex flex-col gap-2">
                                        ...
                                        <div className="mt-2 pt-2 border-t border-gray-100">
                                            <label className="text-[10px] font-bold text-[#091F40] uppercase block mb-1">Tiempo Agregado (Minutos)</label>
                                            <select
                                                value={match.addedTime || 0}
                                                onChange={(e) => updateMatchAddedTime(match.id, e.target.value)}
                                                className="w-full bg-slate-50 border border-slate-200 text-[#091F40] rounded-lg p-2 text-xs font-bold focus:ring-2 focus:ring-[#009B3A] outline-none"
                                            >
                                                {[...Array(16).keys()].map(n => (
